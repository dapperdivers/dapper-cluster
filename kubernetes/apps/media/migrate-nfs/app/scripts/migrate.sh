...
process_directory() {
    ...
    # Count files before transfer for logging (streaming to avoid memory buildup)
    SOURCE_COUNT=$(find "${SOURCE}/${dir_name}" -type f -printf '.' 2>/dev/null | wc -c)
    ...
    # Redirect stdout to /dev/null - verbose/progress output causes OOM when buffered
    # Transfer details are captured via --log-file instead
    if rsync "${rsync_opts[@]}" \
        --log-file="${dir_log}" \
        "${SOURCE}/${dir_name}/" \
        "${DEST}/${dir_name}/" >/dev/null 2>"${rsync_stderr}"; then
